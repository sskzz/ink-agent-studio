...
export const runtimeForeshadowingStatusSchema = z.enum(["planned", "planted", "resolving", "resolved"]);
...
export const runtimeStateSchema = z.object({
  ...
  deltas: z.array(runtimeStateDeltaRecordSchema).max(200).default([]),
  /** 历史快照栈：每章应用前的 state（增量合成：保存 O(1)，不再从 baseline 全量重放）。 */
  history: z.array(runtimeStateHistoryRecordSchema).max(200).default([]),
